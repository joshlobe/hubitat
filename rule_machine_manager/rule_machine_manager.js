...
jQuery( document ).ready( function( $ ) {
	...
	$( document ).on( 'click', 'div.edit_title_div', function() {
		...
		var edit_actions = '<span class="tooltip title_bold_tooltip"><span class="button title_bold"><i class="material-icons">format_bold</i></button><span class="tooltiptext">Title Bold</span></span></span>';
		edit_actions += '<span class="tooltip color_picker_tooltip"><input type="text" class="color_picker" /><span class="tooltiptext">Title Color</span></span>';
		edit_actions += '<span class="button submit_edit">Submit</span>';
		...
		if( title_bold == 'true' ) {
			$( this ).parent().siblings( 'span.group_name_edit' ).children( 'span.title_bold_tooltip' ).children( 'span.title_bold' ).addClass( 'active' );
		}
		...
		$(".color_picker").spectrum({
			...
			change: function( color ) {
				...
				$( this ).parent().parent().parent().siblings( 'input.title_color' ).val( color.toHexString() );
				$( this ).parent().parent().parent().siblings( 'input.title_opacity' ).val( color.getAlpha() );
			}
		});
	});
	...
	$( document ).on( 'click', 'span.title_bold', function() {
		...
		$( this ).parent().parent().parent().siblings( 'input.title_bold' ).val( title_bold );
	});
	...
	$( "span#create_group_button" ).click( function() {
		...
		if( name == '' ) {
			...
			Swal.fire({
				titleText: "Container Title Missing",
				text: "Please first enter a valid container title in the input field.",
				icon: "question",
				iconColor: "#d33",
	...
	$( document ).on( 'click', 'div.delete_container', function() {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				Swal.fire({
					title: "Deleted!",
					text: "The container was deleted successfully.",
					confirmButtonColor: "#81bc00",
					icon: "success",
					allowOutsideClick: false,
					allowEscapeKey: false,
	...
	$( document ).on( 'click', 'i.toggleContainer', function() {
		...
		if( $( this ).text() == 'file_upload' ) {
			...
			$( this ).parent().html( '<i class="material-icons toggleContainer">file_download</i><span class="tooltiptext">Expand</span>' );
		}
		else if( $( this ).text() == 'file_download' ) {
			
			$( this ).parent().html( '<i class="material-icons toggleContainer">file_upload</i><span class="tooltiptext">Collapse</span>' );
		}
	...
	$( document ).on( 'click', 'i.copy_rule', function() {
		...
		Swal.fire({
			...
			text: "Duplicating this rule only makes a duplicate in the context of this app. It does not actually duplicate the rule in the Rule Machine app.",
			icon: "info",
			iconColor: "#81bc00",
			showCancelButton: true,
			confirmButtonColor: "#81bc00",
			...
			if (result.isConfirmed) {
				...
				var new_item = $( thisCopy ).parent().parent().clone();
				$( thisCopy ).parent().parent().after( new_item );
				
				new_item.find( 'span.copy_rule' ).remove();
				var text = "<span class='tooltip delete_duplicate'><i class='material-icons duplicate_rule rule'>delete</i><span class='tooltiptext'>Remove Duplicate Rule</span></span>"
				$( text ).insertAfter( new_item.find( 'span.ruleView' ) );
	...
	// View rule
	$( document ).on( 'click', 'span.ruleView', function() {
	...
	$( document ).on( 'click', 'span.delete_duplicate', function() {
		...
		Swal.fire({
			...
			text: "Removing this duplicate rule only removes it in the context of this app. It does not actually remove the rule in the Rule Machine app.",
			icon: "question",
			iconColor: "#d33",
			showCancelButton: true,
			confirmButtonColor: "#81bc00",
	...
	$( document ).on( 'keyup', 'span.container_filter', function() {
		...
		$( this ).parent().siblings( 'ul.rulelist' ).children().each(function() {
			
			var thisName = $(this).children( 'span.rule_name' ).text();
	...
	$( document ).on( 'keyup', 'span#global_filter', function() {
		...
		$( 'ul.rulelist' ).children().each(function() {
	...
	$( document ).on( 'click', 'div#nag_content i.features.styling', function() {
		...
		var html = '';
		html += '<div class="nag_content">';
			html += '<p>RMM allows styling of the container area and header. Containers can have their background colors styled; helping to visually identify certain containers.</p>';
			html += '<p>Container titles can also be styled independent of each other. Currently supported are title weight, title color and title opacity.</p>';
			html += '<p>Styling the containers and the header area can help facilitate quick visual identification of frequently used rules. Use color combinations which can help visual organization.</p>';
